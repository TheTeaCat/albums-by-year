  ...
  <div class="library">
    ...
    <div v-else>
      <h1 class="library-header">
        ...
        >{{
          profile.display_name[profile.display_name.length - 1] == "s"
            ? "'"
            : "'s"
        }}
      ...
      <div class="searchbox-cont">
        ...
        />
        <font-awesome-icon
          v-if="searchBoxContent != search_query || searchBoxContent == ''"
          icon="search"
          class="icon fa-fw"
      ...
      <ol>
        <Year
          v-for="year in sortedAlbumYears"
          :key="year"
          :year="year"
      ...
      <h2 v-show="loadedAlbums.length == 0" class="empty-notice">
        You haven't got any saved albums!
      </h2>
      <h2 v-show="no_albums_shown" class="empty-notice">
        None of your albums match these filters!
      </h2>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      profile: null,
      album_types: {
        album: { show: true, display: "Albums" },
        single: { show: false, display: "Singles" },
        compilation: { show: false, display: "Compilations" },
      },
      loadedAlbums: 0,
      search_query: "",
      searchBoxContent: "",
  ...
  computed: {
    albums_request_url() {
      return "https://api.spotify.com/v1/me/albums?" + "&limit=50" + "&offset=0";
    },
    profile_request_url() {
      return "https://api.spotify.com/v1/me/?";
    },
    sortedAlbumYears() {
      return Object.keys(this.albumsByYear).sort((a, b) => {
        return b - a;
      });
    },
    loaded() {
  ...
  methods: {
    ...
    searchQueryChange(event) {
      ...
      setTimeout(() => {
        value == this.searchBoxContent ? this.updateSearchQuery(event) : null;
      }, 1000);
    },
    clearSearchQuery() {
    ...
    checkAlbumsShown() {
      this.no_albums_shown = Object.keys(this.albumsByYear)
        .map((year) => this.albumsByYear[year].empty)
        .every((x) => x);
    },
  },
